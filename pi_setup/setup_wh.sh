#!/bin/bash
# KidBot Pi Zero WH setup script
# Run on a fresh Raspberry Pi OS Lite (32-bit) install
# Usage: cd ~/kidbot && sudo bash pi_setup/setup_wh.sh
#
# Pi Zero WH notes:
#   - ARMv6 single-core 700 MHz — must use 32-bit OS
#   - DISPLAY_FPS=8 (raise to 10 only if audio stays clean)
